...
enum InsigniaAuthService {
    ...
    private static var specialAccessToken: String {
    ...
    static func login(email: String, password: String) async throws -> (sessionKey: String, username: String, email: String?) {
        ...
        request.httpBody = try JSONEncoder().encode(
            LoginRequest(email: email, password: password, specialAccess: specialAccessToken)
        )
...
private enum ProfileSessionKeychain {
    ...
    private static let fallbackDirectoryName = "DukeX"
    private static let fallbackFileName = "xb-live-session.key"
    ...
    static func saveSessionKey(_ sessionKey: String) throws {
        ...
        if status == errSecSuccess {
            try? deleteFallbackSessionKey()
            return
        }

        #if targetEnvironment(macCatalyst) || os(macOS)
        try saveFallbackSessionKey(data)
        #else
    ...
    static func loadSessionKey() throws -> String? {
        ...
        if status == errSecItemNotFound {
            return try loadFallbackSessionKey()
        }
    ...
    private static var fallbackSessionURL: URL {
        let baseURL = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        return baseURL
            .appendingPathComponent(fallbackDirectoryName, isDirectory: true)
            .appendingPathComponent(fallbackFileName, isDirectory: false)
    }

    private static func saveFallbackSessionKey(_ data: Data) throws {
        let directoryURL = fallbackSessionURL.deletingLastPathComponent()
        try FileManager.default.createDirectory(
            at: directoryURL,
            withIntermediateDirectories: true,
            attributes: [.posixPermissions: 0o700]
        )
        try data.write(to: fallbackSessionURL, options: [.atomic, .completeFileProtection])
        chmod(directoryURL.path, 0o700)
        chmod(fallbackSessionURL.path, 0o600)
    }

    private static func loadFallbackSessionKey() throws -> String? {
        let url = fallbackSessionURL
    ...
    private static func deleteFallbackSessionKey() throws {
        let url = fallbackSessionURL
        guard FileManager.default.fileExists(atPath: url.path) else {
            return
        }
