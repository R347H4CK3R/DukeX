...
struct ProfileSocialMessagesToolbox: View {
    ...
    var body: some View {
        ...
        .onChange(of: conversations.map(\.key)) { _ in
            normalizeSelectedConversation()
        }
        .onChange(of: conversationSearchText) { _ in
            normalizeSelectedConversationForFilter()
        }
        .sheet(isPresented: $isComposePresented) {
            NavigationStack {
                ProfileSocialComposeView(
    ...
    private var toolboxContent: some View {
        VStack(spacing: 12) {
            ...
            if conversations.isEmpty {
                ...
                Spacer(minLength: 0)
                ProfileEmptyRow(title: "No matching conversations", systemImage: "magnifyingglass")
                Spacer(minLength: 0)
            } else {
                ScrollView {
                    LazyVStack(spacing: 8) {
                        ForEach(displayedConversations) { conversation in
                            Button {
                                ...
                                ProfileSocialDesktopConversationRow(
                                    conversation: conversation,
                                    avatar: avatarResolver.avatar(
                                        for: conversation.username,
                                        fallbackURL: conversation.avatarURL
                                    ),
                                    isSelected: conversation.key == selectedConversationKey
                                )
                            }
                            .buttonStyle(.plain)
                            .contextMenu {
    ...
    private var displayedConversations: [XBLiveSocialConversation] {
        ...
        return conversations.filter { conversation in
            conversation.username.localizedCaseInsensitiveContains(query) ||
                (conversation.latestBody?.localizedCaseInsensitiveContains(query) == true)
        }
    }
    ...
    private func normalizeSelectedConversation() {
        ...
        if let selectedConversationKey,
           conversations.contains(where: { $0.key == selectedConversationKey }) {
            return
        }

        selectedConversationKey = displayedConversations.first?.key ?? conversations.first?.key
    }

    private func normalizeSelectedConversationForFilter() {
        guard !displayedConversations.isEmpty else {
            return
        }

        if let selectedConversationKey,
           displayedConversations.contains(where: { $0.key == selectedConversationKey }) {
            return
        }
...
#if targetEnvironment(macCatalyst)
private struct ProfileSocialDesktopConversationRow: View {
    let conversation: XBLiveSocialConversation
    let avatar: ProfileSocialAvatarData
    let isSelected: Bool
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(detail)
                    ...
                    .foregroundStyle(isSelected ? Color.primary.opacity(0.86) : Color.secondary)
                    .lineLimit(2)
            ...
            if let latestAt = ProfileSocialTimestamp.displayText(conversation.latestAt) {
                Text(latestAt)
                    ...
                    .foregroundStyle(isSelected ? Color.primary.opacity(0.80) : Color.secondary)
                    .lineLimit(1)
            }
        }
        .padding(.horizontal, 10)
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .strokeBorder(isSelected ? Color.accentColor.opacity(0.38) : Color.primary.opacity(0.06), lineWidth: 1)
        }
    ...
    private var rowBackground: some View {
        RoundedRectangle(cornerRadius: 12, style: .continuous)
            .fill(isSelected ? Color.accentColor.opacity(0.24) : Color.primary.opacity(0.035))
    }
