...
# Export a tiny preflight entry point which Swift calls on @MainActor before the
# background xemu loop starts.
...
# Never let the background emulator thread enter SDL's UIKit video bootstrap.
# If main-thread preflight did not happen, fail loudly instead of hanging forever.
...
if "XemuPrepareSDLVideo" not in swift:
    ...
    swift = replace_once(
        ...
        "Swift SDL preflight loader call",
    )
