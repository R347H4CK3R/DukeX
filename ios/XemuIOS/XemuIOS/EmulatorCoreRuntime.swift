...
final class EmulatorCoreRuntime: ObservableObject {
    ...
    private static func invoke(
        ...
        let useVulkanSwapchain = bundleIdentifier.hasPrefix("com.mafty.DukeX") ||
            bundleIdentifier.hasPrefix("com.michaelweekley.DukeX")
        ...
        setenv("XEMU_IOS_VK_PRESENT_FPS", presentPacingMode.presentFPS, 1)
        setenv("XEMU_IOS_VK_PRESENT_MODE", presentPacingMode.vulkanPresentMode, 1)
        setenv("XEMU_IOS_DISPLAY_SYNC", presentPacingMode.displaySyncEnabled ? "1" : "0", 1)
        setenv("XEMU_IOS_NOMINAL_FPS", presentPacingMode.nominalFramesPerSecond, 1)
        ...
        NSLog(
            "XEMU_IOS_PRESENT_PACING=%@ mode=%@ displaySync=%@ nominalFPS=%@ presentFPS=%@",
            presentPacingMode.rawValue,
            presentPacingMode.vulkanPresentMode,
            presentPacingMode.displaySyncEnabled ? "1" : "0",
            presentPacingMode.nominalFramesPerSecond,
            presentPacingMode.presentFPS
        )
...
private enum PresentPacingLayerConfigurator {
    static func apply(to layer: CAMetalLayer) {
        ...
        layer.setBoolWithSelector("setDisplaySyncEnabled:", value: mode.displaySyncEnabled)
        layer.setIntWithSelector("setNominalFramesPerSecond:", value: mode.nominalFramesPerSecondValue)
        NSLog(
            "Native Metal presenter pacing applied: %@ displaySync=%@ nominalFPS=%@",
            mode.rawValue,
            mode.displaySyncEnabled ? "1" : "0",
            mode.nominalFramesPerSecond
        )
    }
}
...
private final class NativeMetalPresenterHost {
