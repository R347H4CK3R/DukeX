...
#ifdef CONFIG_IOS
    static unsigned ios_pramdac_write_logs;
    if (ios_nv2a_write_trace_enabled() && ios_pramdac_write_logs < 80) {
        fprintf(stderr,
                "xemu_ios: pramdac write[%u] addr=0x%" HWADDR_PRIx
                " size=%u val=0x%016" PRIx64 "\n",
                ios_pramdac_write_logs++, addr, size, val);
    }
#endif
